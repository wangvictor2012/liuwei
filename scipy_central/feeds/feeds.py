...
class RssSiteFeed(Feed):
    title = "Recent Submissions - ABgenomica's Publication"
    link = "/item/show/all-revisions/"
    feed_url = '/feeds/'
    description = "Most recent 30 submissions from ABgenomica's Publication"
    description_template = "feeds/item_description.html"
    feed_copyright = 'Please visit http://abgenomica.com link of ABgenomica Website'
...
class RssSubmissionFeed(RssSiteFeed):
    ...
    def title(self, obj):
        ...
        return '%s - ABgenomica Publications' % rev_obj.title
...
class RssTagFeed(RssSiteFeed):
    ...
    def title(self, obj):
        return 'Recent 30 %s Submissions - ABgenomica Publication' % obj.slug
